...
import com.iktwo.juntalos.R;

public class Juntalos extends org.qtproject.qt5.android.bindings.QtActivity
{
    private static final String TAG = "Juntalos";
    private static Juntalos m_instance;
    ...
    public static void shareUrl(String name, String url)
    {
        ...
        if (!name.isEmpty()) {
            share.putExtra(Intent.EXTRA_TEXT, name + " - " + url + " via Juntalos");
            m_instance.startActivity(Intent.createChooser(share, "Share " + name));
        } else {
            share.putExtra(Intent.EXTRA_TEXT, url + " via Juntalos");
